---
last_mapped: 2026-02-25T12:00:00Z
total_files: 6
total_tokens: 11042
---
...
> Auto-generated by Cartographer. Last mapped: February 25, 2026
...
graph TD
    ...
    SubagentSpawn --> Subagents["Explore Subagents (inline prompts)"]
    Subagents -- "Module Analysis" --> Skill
...
| File | Purpose | Tokens |
|------|---------|--------|
| SKILL.md | Orchestration workflow & instructions | 2,176 |
...
- Cursor Task tool API (subagent prompts are inline)
...
sequenceDiagram
    ...
    par Parallel Subagent Execution
        Skill->>Task: Spawn Subagent 1 (inline prompt, Group A files)
        Task-->>Skill: Structured markdown report
    and
        Skill->>Task: Spawn Subagent 2 (inline prompt, Group B files)
        Task-->>Skill: Structured markdown report
    and
...
- **Subagent Delegation**: The main agent MUST NEVER read codebase files directly; it always delegates to subagents (spawned with inline prompts) to prevent context overflow.
...
1. Edit the subagent prompt template in `.cursor/skills/cartographer/SKILL.md`
2. Subagent prompts are defined inline within the orchestrator workflow
3. Use `subagent_type: "explore"` with `model: "fast"` for analysis tasks
